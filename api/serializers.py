...
class CalenderSerializer(serializers.ModelSerializer):
    ...
    def delete(self,data):
        ...
        calender_needed_to_delete = Calender.obejcts.filter(user = user).filter(contest = data.contest_id)
        Test_needed_to_delete = Test.objects.filter(user = user).filter(contest_id = data.contest_id)
        calender_needed_to_delete.save()
        Test_needed_to_delete.save()
        return Calender.objects.filter(user = user)
